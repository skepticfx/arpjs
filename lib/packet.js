...
// TODO: Need to do something about interface. Works on OSX for now.
var iface = "en0";

var pcap = require("pcap");
var session = pcap.createSession(iface, "");
...
var send = function(pkt_o){
  ...
  require('getmac').getMac(function(err, macAddr){
    ...
    var pkt = {};
    pkt.dst_mac = mac_to_arr("ff:ff:ff:ff:ff:ff");
    ...
    pkt.src_ip = ip_to_arr("192.168.100.1");
    if(pkt_o.hasOwnProperty('src_ip'))
      pkt.src_ip = mac_to_arr(pkt_o.src_ip);
    ...
    if(pkt_o.hasOwnProperty('dst_ip'))
      pkt.dst_ip = mac_to_arr(pkt_o.dst_ip);
